...
<% 
	...
	String output_id = request.getParameter("id");
	String sql = "DELETE FROM `web_terminal`.`record_output` WHERE (`output_id` = ?)";
	Connection con = null;
	...
	try{
		...
		ps.executeUpdate();
		out.print("<script type='text/javascript'>alert('订单信息删除成功!');window.location.href='outstock.jsp?pages=1';</script>");
		ps.close();
		con.close();
	}catch(Exception e){
		e.printStackTrace();
